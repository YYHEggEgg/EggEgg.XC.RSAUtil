...
namespace XC.RSAUtil.Test
{
    class Program
    {
        static void Main(string[] args)
        {
			...
			Console.WriteLine(RsaKeyGenerator.XmlKey(2048).privateKey);
            Console.WriteLine(RsaKeyGenerator.Pkcs1Key(2048,true).privateKey);
            Console.WriteLine(RsaKeyGenerator.Pkcs8Key(2048,true).privateKey);


            Console.WriteLine("Key Convert:");
            var keyList = RsaKeyGenerator.Pkcs1Key(2048, false);
            var privateKey = keyList.privateKey;
            var publicKey = keyList.publicKey;
            Console.WriteLine("public key pkcs1->xml:");
            Console.WriteLine(RsaKeyConvert.PublicKeyPemToXml(publicKey));

            var bigDataRsa=new RsaPkcs1Util(privateKey, keySize: 2048);
            ...
            var str = bigDataRsa.RsaEncrypt(Encoding.UTF8.GetBytes(data), RSAEncryptionPadding.Pkcs1);
            Console.WriteLine("Big Data Encrypt:");
            Console.WriteLine(Convert.ToBase64String(str));
            Console.WriteLine("Big Data Decrypt:");
            Console.WriteLine(Encoding.UTF8.GetString(bigDataRsa.RsaDecrypt(str,  RSAEncryptionPadding.Pkcs1)));
